...
import tableModels.readingsTableModels.CONDReadingTableModel;
import wizard.WizardPanel;

public class CONDReadingsWizardPanel extends WizardPanel implements ReadingsPanel {
	...
	private CONDReadingTableModel condReadingTableModel;
	...
	public CONDReadingsWizardPanel(){
		init();
	}
	/**
	 * Create the panel.
	 */
	public CONDReadingsWizardPanel(double maxRange, double acceptanceLimit, int type) {
	...
	private void init(){
		...
		condReadingTableModel = new CONDReadingTableModel();
		
		setPreferredSize(new Dimension(646, 335));
		setLayout(null);

		JLabel lblTemperature = new JLabel("Conductivity Calibration Result");
		...
		addButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				CondCertificateLine line = new CondCertificateLine();
				...
				CONDReadingTableModel model = (CONDReadingTableModel) table
						.getModel();
				model.getCondCertificateLinesList().add(line);
		...
		addButton.setIcon(new ImageIcon(CONDReadingsWizardPanel.class
		...
		removeButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (row >= 0) {
					CONDReadingTableModel tableModel = (CONDReadingTableModel) table
							.getModel();
					tableModel.getCondCertificateLinesList().remove(row);
					tableModel.fireTableDataChanged();
				} else {
					JOptionPane
							.showMessageDialog(CONDReadingsWizardPanel.this,
		...
		removeButton.setIcon(new ImageIcon(CONDReadingsWizardPanel.class
		...
		removeAllButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				CONDReadingTableModel tableModel = (CONDReadingTableModel) table
						.getModel();
				if (tableModel.getCondCertificateLinesList().size() > 0) {
					tableModel.getCondCertificateLinesList().removeAll(
							tableModel.getCondCertificateLinesList());
					tableModel.fireTableDataChanged();
				} else {
					JOptionPane
							.showMessageDialog(CONDReadingsWizardPanel.this,
		...
		removeAllButton.setIcon(new ImageIcon(CONDReadingsWizardPanel.class
		...
		table = new JXTable(condReadingTableModel);
	...
	public List<CondCertificateLine> getCondLinesList() {
		CONDReadingTableModel model = (CONDReadingTableModel) table.getModel();
		return model.getCondCertificateLinesList();
	}
