...
import tableModels.readingsTableModels.TSReadingTableModel;
import wizard.WizardPanel;
import formatters.ImprovedFormattedTextField;

public class TSReadingsWizardPanel extends WizardPanel  implements ReadingsPanel {
	...
	private JFormattedTextField settingField;
	...
	private TSReadingTableModel tsReadingTableModel;

	/**
	 * Create the panel.
	 */
	public TSReadingsWizardPanel() {

		NumberFormat decimalFormat = NumberFormat.getInstance();
		decimalFormat.setMaximumFractionDigits(10);

		tsReadingTableModel = new TSReadingTableModel();
		...
		settingField = new ImprovedFormattedTextField(decimalFormat);
		panel_1.add(settingField);
		settingField.setColumns(7);
		...
		addButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				TSCertificateLine line = new TSCertificateLine();
				line.setSettingReading((long) settingField.getValue());
				line.setReferenceReading((long) referenceField.getValue());
				line.setUnitUnderTestReading((long) unitField.getValue());

				TSReadingTableModel model = (TSReadingTableModel) table
						.getModel();
				model.getTSCertificateLinesList().add(line);
				model.fireTableDataChanged();
			}
		});

		addButton.setPreferredSize(new Dimension(30, 30));
		addButton.setIcon(new ImageIcon(TSReadingsWizardPanel.class
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (selectedRows.length > 0) {
					...
					for (int row : selectedRows) {
						TSReadingTableModel tableModel = (TSReadingTableModel) table
								.getModel();
						tableModel.getTSCertificateLinesList().remove(row);

					}
					
				} else {
					JOptionPane
							.showMessageDialog(TSReadingsWizardPanel.this,
		...
		removeButton.setIcon(new ImageIcon(TSReadingsWizardPanel.class
		...
		removeAllButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				TSReadingTableModel tableModel = (TSReadingTableModel) table
						.getModel();
				if (tableModel.getTSCertificateLinesList().size() > 0) {
					tableModel.getTSCertificateLinesList().removeAll(
							tableModel.getTSCertificateLinesList());
		...
		removeAllButton.setIcon(new ImageIcon(TSReadingsWizardPanel.class
		...
		table = new JXTable(tsReadingTableModel);
	...
	public List<TSCertificateLine> getTSLinesList() {
		TSReadingTableModel model = (TSReadingTableModel) table.getModel();
		return model.getTSCertificateLinesList();
	}
	...
	public void setLimitCalculationType(int method){
		this.limitCalculationType = method;
	}
