...
import tableModels.readingsTableModels.PDGReadingTableModel;
import wizard.WizardPanel;

public class PDGReadingsWizardPanel extends WizardPanel implements ReadingsPanel {
	...
	private PDGReadingTableModel pdgReadingTableModel;
	...
	public PDGReadingsWizardPanel(){
		init();
	}
	/**
	 * Create the panel.
	 */
	public PDGReadingsWizardPanel(double maxRange, double acceptanceLimit, int type) {
	...
	private void init(){
		...
		pdgReadingTableModel = new PDGReadingTableModel();
		
		setPreferredSize(new Dimension(646, 335));
		setLayout(null);

		JLabel lblTemperature = new JLabel("Differential Pressure Calibration Result");
		...
		addButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				PDGCertificateLine line = new PDGCertificateLine();
				...
				PDGReadingTableModel model = (PDGReadingTableModel) table
						.getModel();
				model.getPDGCertificateLinesList().add(line);
		...
		addButton.setIcon(new ImageIcon(PDGReadingsWizardPanel.class
		...
		removeButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (row >= 0) {
					PDGReadingTableModel tableModel = (PDGReadingTableModel) table
							.getModel();
					tableModel.getPDGCertificateLinesList().remove(row);
					tableModel.fireTableDataChanged();
				} else {
					JOptionPane
							.showMessageDialog(PDGReadingsWizardPanel.this,
		...
		removeButton.setIcon(new ImageIcon(PDGReadingsWizardPanel.class
		...
		removeAllButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				PDGReadingTableModel tableModel = (PDGReadingTableModel) table
						.getModel();
				if (tableModel.getPDGCertificateLinesList().size() > 0) {
					tableModel.getPDGCertificateLinesList().removeAll(
							tableModel.getPDGCertificateLinesList());
					tableModel.fireTableDataChanged();
				} else {
					JOptionPane
							.showMessageDialog(PDGReadingsWizardPanel.this,
		...
		removeAllButton.setIcon(new ImageIcon(PDGReadingsWizardPanel.class
		...
		table = new JXTable(pdgReadingTableModel);
	...
	public List<PDGCertificateLine> getPDGLinesList() {
		PDGReadingTableModel model = (PDGReadingTableModel) table.getModel();
		return model.getPDGCertificateLinesList();
	}
